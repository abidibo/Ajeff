...
function cleanInput($method, $name, $type, $opts=array()) {
	...
	if($method == 'get' && isset($_GET[$name])) return $_GET[$name];
	elseif($method == 'post' && isset($_POST[$name])) return $_POST[$name];
	elseif($method == 'request' && isset($_REQUEST[$name])) return $_REQUEST[$name];
